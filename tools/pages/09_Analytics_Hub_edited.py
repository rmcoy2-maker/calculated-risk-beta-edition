...
def load_team_market_outperformance() -> pd.DataFrame:
    root = _exports_dir()
    ...
    home = df.groupby("home").agg(
        actual=("home_win", "sum"),
        expected=("expected_win_home", "sum"),
        games=("home_win", "count"),
    )
    away = df.groupby("away").agg(
        actual=("away_win", "sum"),
        expected=("expected_win_away", "sum"),
        games=("away_win", "count"),
    )

    teams_df = home.add(away, fill_value=0)
    teams_df["diff"] = teams_df["actual"] - teams_df["expected"]
    teams_df["tier"] = "Average"
    teams_df.loc[teams_df["diff"] >= 1.5, "tier"] = "Overperformer"
    teams_df.loc[teams_df["diff"] <= -1.5, "tier"] = "Underperformer"

    teams_df = teams_df.reset_index().rename(columns={"index": "team", "home": "team", "away": "team"})
    teams_df["actual"] = teams_df["actual"].round(2)
    teams_df["expected"] = teams_df["expected"].round(2)
    teams_df["diff"] = teams_df["diff"].round(2)
    teams_df["games"] = teams_df["games"].astype(int)

    return teams_df.sort_values("diff", ascending=False).reset_index(drop=True)
...
if mode == "Team Profile":
    ...
    market_perf = load_team_market_outperformance()
    ...
    if market_perf.empty:
        ...
        team_market_perf = market_perf.loc[market_perf["team"] == team].copy()
        if team_market_perf.empty:
            st.info(f"No market-based team outperformance rows were found for {team}.")
        else:
