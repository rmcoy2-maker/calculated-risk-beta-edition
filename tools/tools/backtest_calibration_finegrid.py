...
OUT_DIR = PROJECT_ROOT / "exports" / "backtest_calibration_finegrid"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EDGE_THRESHOLDS = [0.03, 0.04, 0.05, 0.06, 0.07, 0.08]
SHRINK_FACTORS = [0.85, 0.88, 0.90, 0.92, 0.95, 1.00]
...
def summarize_thresholds(
    ...
    for th in thresholds:
        ...
        max_possible_profit = float(sub["profit_per_bet_cap"].sum()) if n else 0.0
        ...
        rows.append(
            {
                ...
                "max_possible_profit_if_all_won": max_possible_profit,
            }
        )
...
def main() -> None:
    ...
    # sanity cap for auditing: per-bet win profit cannot exceed ~1.10 in this constrained universe
    bets["profit_per_bet_cap"] = bets["profit"].where(bets["profit"] > 0, 0.0)
    ...
    print(
        bets[
            ["game_id", "Season", "side", "closing_odds", "raw_model_prob", "actual_win", "profit"]
        ]
        .head(10)
    ...
    # hard sanity check
    if bets["profit"].abs().max() > 2.0:
        raise ValueError(
            f"Unexpected profit magnitude detected: max abs(profit) = {bets['profit'].abs().max():.4f}. "
            "Profit should be near [-1, 1.10] in this constrained universe."
        )

    all_results = []

    for factor in SHRINK_FACTORS:
        all_results.append(summarize_thresholds(bets, EDGE_THRESHOLDS, factor, "all"))
        all_results.append(summarize_thresholds(bets[bets["Season"] == 2024].copy(), EDGE_THRESHOLDS, factor, "2024"))
        all_results.append(summarize_thresholds(bets[bets["Season"] == 2025].copy(), EDGE_THRESHOLDS, factor, "2025"))

    results = pd.concat(all_results, ignore_index=True)
    results.to_csv(OUT_DIR / "finegrid_results.csv", index=False)
    ...
    print("\nTOP ALL SEASONS")
    print(best_all.head(20).to_string(index=False))

    print("\nTOP 2025")
    print(best_2025.head(20).to_string(index=False))
